...
// from https://github.com/mapbox/eternal/blob/master/include/mapbox/eternal.hpp
...
class element {
  public:
    ...
    constexpr element(const key_type &key,
                      const mapped_type &value) noexcept : pair(key, value) {
    }
...
class compare_key_hash : public compare_key<Key> {
  ...
  public:
    ...
    constexpr compare_key_hash(const Key &key_) noexcept : base_type(key_),
        hash(Hasher()(key_)) {
    }
...
MAPBOX_ETERNAL_CONSTEXPR auto bound(Iterator left, Iterator right,
...
class map {
  ...
  public:
    template <typename T>
    MAPBOX_ETERNAL_CONSTEXPR map(const T (&data)[N]) noexcept : map(data,
    ...
    MAPBOX_ETERNAL_CONSTEXPR const mapped_type &at(const key_type &key) const
    noexcept {
    ...
    MAPBOX_ETERNAL_CONSTEXPR const_iterator lower_bound(const key_type &key) const
    noexcept {
        return bound<less>(data_, data_ + N, compare_key_type{ key });
    }

    MAPBOX_ETERNAL_CONSTEXPR const_iterator upper_bound(const key_type &key) const
    noexcept {
        return bound<greater_equal>(data_, data_ + N, compare_key_type{ key });
    }

    MAPBOX_ETERNAL_CONSTEXPR std::pair<const_iterator, const_iterator> equal_range(
    ...
    MAPBOX_ETERNAL_CONSTEXPR const_iterator find(const key_type &key) const
    noexcept {
        const compare_key_type compare_key{ key };
...
static constexpr auto map(const std::pair<const Key, const Value>
                          (&items)[N]) noexcept {
    return impl::map<impl::element<Key, Value>, N>(items);
}

template <typename Key, typename Value, std::size_t N>
static constexpr auto hash_map(const std::pair<const Key, const Value>
                               (&items)[N]) noexcept {
...
constexpr static std::size_t str_hash(const char *str,
    ...
    return *str ? str_hash(str + 1,
                           (value ^ static_cast<std::size_t>(*str)) * hash_prime) : value;
}
